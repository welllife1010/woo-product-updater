...
async function ensureCategoryHierarchy(resolvedCategory) {
  ...
  logInfoToFile(`[DEBUG] ensureCategoryHierarchy called with: ${JSON.stringify(resolvedCategory)}`);
  
  if (!resolvedCategory || !resolvedCategory.main) {
    logInfoToFile(`[DEBUG] ensureCategoryHierarchy returning null - no main category`);
    // Nothing we can do
    return null;
  }
  ...
  logInfoToFile(`[DEBUG] ensureCategoryHierarchy returning ids: ${JSON.stringify(ids)}`);
  return {
    ids,
    mainId: mainCat.id,
